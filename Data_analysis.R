...
# Theme for graphs stored in my_theme 
...
ggplot(storms,aes(x = wind, y = pressure, color = wind)) +
  geom_point() +
  geom_jitter(width = 4, size = 1,alpha = 0.5) + 
  geom_smooth(method = "gam")+
  xlab("Wind (knots)") +
  ylab("Pressure (millibars)") + 
...
#2. Ordered Bi-variate bar plot of wind vs month.

month_wind <- storms %>% 
  group_by(month) %>%
  summarize(count = n())

month_wind$month  <- with(month_wind,reorder(month, -(count)))
...
# 9th month (September), and the months from August - October 
# have the maximum number of occurrences. 
